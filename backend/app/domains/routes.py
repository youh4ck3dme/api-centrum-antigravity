...
@router.post("/domains")
async def create_domain(domain: DomainCreate, current_user: User = Depends(get_current_user_or_neon)):
    """Create new domain via Websupport API"""
    try:
        return DomainService.create_domain(domain.dict())
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete_domain(domain_id: int, current_user: User = Depends(get_current_user_or_neon)):
    """Delete domain via Websupport API"""
    try:
        return DomainService.delete_domain(domain_id)
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
